...
# === ИНТЕРФЕЙС STREAMLIT (структура как в отчёте) ===
...
with st.form("main_form"):
    ...
    # === ОСНОВНАЯ ИНФОРМАЦИЯ (отдельная секция ПЕРЕД разделом 1) ===
    ...
    # Риски и рекомендации — под таблицами
    st.text_area("Основной риск", "Уязвимости безопасности позволяют нарушителю получить доступ к данным пользователей и вызвать отказ в обслуживании.")
    st.text_area("Рекомендация", "Релиз возможен только после устранения всех S1/S2 дефектов и повторного тестирования.")    
    ...
    for i in range(num_modules):
        with st.expander(f"Модуль 3.{i+1}", expanded=True):
            ...
            # ВАЖНО: в комментариях используется точка после ID бага: "BUG-XXX."
            df = st.data_editor(
                default_df,
                num_rows="dynamic",
                key=df_key,
    ...
    # ВАЖНО: в образце используется нумерованный список (1., 2., 3.)
